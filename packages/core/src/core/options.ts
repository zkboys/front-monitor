import { validateOption, _support, setSilentFlag } from '@front-monitor/utils';
...
export class Options {
  ...
  reportOnlyOnce = false; // 是否去除重复的代码错误，重复的错误只上报一次
  constructor() {}

  bindOptions(options: InitOptions): void {
    const { dsn, filterXhrUrlRegExp, throttleDelayTime = 0, overTime = 10, silentWhiteScreen = false, whiteBoxElements = ['html', 'body', '#app', '#root'], skeletonProject = false, handleHttpStatus, reportOnlyOnce = false } = options;
    ...
    validateOption(reportOnlyOnce, 'reportOnlyOnce', 'boolean') && (this.reportOnlyOnce = reportOnlyOnce);
  }
}
